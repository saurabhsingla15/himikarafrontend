...
function transformer(file, api, options) {
  ...
  function convertToModuleExports(path, originalExports, counter) {
    ...
    adds.forEach(add => {
      ...
      let key = (0, _utils.sanitizeName)(name);
  ...
  root.find(j.CallExpression).filter(add => add.node.callee.property && add.node.callee.property.name === 'add').filter(add => add.node.arguments.length >= 2 && add.node.arguments[0].type === 'Literal').filter(add => add.parentPath.node.type === 'ExpressionStatement').forEach((path, i) => convertToModuleExports(path, originalExports, i)); // remove storiesOf import
